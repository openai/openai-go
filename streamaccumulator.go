...
const (
	nothing chatCompletionResponseStateEnum = iota
	content
	refusal
	tool
	finished
...
func (acc *ChatCompletionAccumulator) JustFinishedContent() (string, bool) {
	if acc.justFinished.state == content {
...
// an empty string is returned and the boolean will be false.
func (acc *ChatCompletionAccumulator) JustFinishedRefusal() (string, bool) {
	if acc.justFinished.state == refusal {
...
func (acc *ChatCompletionAccumulator) JustFinishedToolCall() (FinishedChatCompletionToolCall, bool) {
	if acc.justFinished.state == tool {
