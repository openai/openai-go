...
type UnionWithDates struct {
	...
	OfDateTime param.Opt[time.Time]
}

func (r UnionWithDates) MarshalJSON() (data []byte, err error) {
	return param.MarshalUnion[UnionWithDates](param.EncodedAsDate(r.OfDate), r.OfDateTime)
}
...
func TestUnionDateMarshal(t *testing.T) {
	tests := map[string]struct {
		...
		"datetime_only": {
			UnionWithDates{
				OfDateTime: param.Opt[time.Time]{Value: time.Date(2023, time.March, 18, 14, 47, 38, 0, time.UTC)},
			},
			`"2023-03-18T14:47:38Z"`,
		},
	}
