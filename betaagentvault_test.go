...
func TestBetaAgentVaultNewWithOptionalParams(t *testing.T) {
	...
	_, err := client.Beta.Agents.Vaults.New(context.TODO(), openai.BetaAgentVaultNewParams{
...
func TestBetaAgentVaultGet(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Beta.Agents.Vaults.Get(context.TODO(), "vault_id")
	if err != nil {
		var apierr *openai.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestBetaAgentVaultDelete(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Beta.Agents.Vaults.Delete(context.TODO(), "vault_id")
	if err != nil {
		var apierr *openai.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
