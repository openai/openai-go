...
func TestRetryAfterDelayEdgeCases(t *testing.T) {
	tests := map[string]struct {
		...
		"numeric seconds overflow":                  {header: http.Header{"Retry-After": {strings.Repeat("9", 400)}}, wantAttempts: 1},
		"numeric milliseconds overflow":             {header: http.Header{"Retry-After-Ms": {strings.Repeat("9", 400)}}, wantAttempts: 1},
		"exponent overflow":                         {header: http.Header{"Retry-After": {"1e999"}}, wantAttempts: 1},
		"negative overflow uses bounded backoff":    {header: http.Header{"Retry-After": {"-1e999"}}, maxRetryDelay: 10 * time.Millisecond, wantAttempts: 2, minimumWait: 7500 * time.Microsecond, maximumWait: 10 * time.Millisecond},
		"literal infinity uses bounded backoff":     {header: http.Header{"Retry-After": {"+Inf"}}, maxRetryDelay: 10 * time.Millisecond, wantAttempts: 2, minimumWait: 7500 * time.Microsecond, maximumWait: 10 * time.Millisecond},
