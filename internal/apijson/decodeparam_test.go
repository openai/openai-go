...
type MessageVariantSuperset struct {
	ID      string `json:"id" api:"required"`
	Type    string `json:"type" api:"required"`
	Role    string `json:"role" api:"required"`
	Content string `json:"content" api:"required"`
...
func (m *MessageVariantSuperset) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, m)
}

type SupersetMessageUnion struct {
	OfInput    *MessageVariantInput    `json:",inline"`
	OfSuperset *MessageVariantSuperset `json:",inline"`

	paramUnion
}

func (u *SupersetMessageUnion) UnmarshalJSON(data []byte) error {
...
func init() {
	...
	apijson.RegisterUnion[SupersetMessageUnion](
		"type",
		apijson.Discriminator[MessageVariantInput]("message"),
		apijson.Discriminator[MessageVariantSuperset]("message"),
	)
}
...
// A variant that recognizes more of the present keys must not win when it is
// missing required keys that another candidate does not require.
func TestDiscriminatorTiebreakPrefersRequiredFields(t *testing.T) {
	var dst SupersetMessageUnion
	raw := `{"type":"message","role":"user","content":"hi","status":"x"}`
	if err := json.Unmarshal([]byte(raw), &dst); err != nil {
		t.Fatalf("failed unmarshal with err: %v", err)
	}
	if dst.OfSuperset != nil || dst.OfInput == nil {
		t.Fatalf("expected OfInput to win when id is absent, got %#v", dst)
	}

	raw = `{"id":"msg_1","type":"message","role":"user","content":"hi","status":"x"}`
	dst = SupersetMessageUnion{}
	if err := json.Unmarshal([]byte(raw), &dst); err != nil {
		t.Fatalf("failed unmarshal with err: %v", err)
	}
	if dst.OfInput != nil || dst.OfSuperset == nil {
		t.Fatalf("expected OfSuperset to win when all required keys are present, got %#v", dst)
	}
}
