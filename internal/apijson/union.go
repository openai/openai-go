...
// including those contributed by embedded anonymous structs. The value reports
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if field.Anonymous {
			if field.Type.Kind() == reflect.Struct {
				for key, required := range knownJSONKeys(field.Type) {
					keys[key] = required
				}
			}
			continue
	...
	return func(n gjson.Result, v reflect.Value, state *decoderState) error {
		if discriminated && n.Type == gjson.JSON && len(unionEntry.discriminatorKey) != 0 {
			...
			if len(matches) > 1 {
				...
				// match.
				// A candidate missing required keys loses to one that has them all,
				// even if it recognizes more of the keys that are present. Unknown
				// keys break ties.
				nodeMap := n.Map()
				bestMissing, bestUnknown := -1, -1
				var winner *discriminatedDecoder
				for i := range matches {
					decoder := matches[i]
					known := knownJSONKeys(decoder.field.Type.Elem())
					missing := 0
					for key, required := range known {
						if _, ok := nodeMap[key]; required && !ok {
							missing++
						}
					}
					unknown := 0
					...
					if winner == nil || missing < bestMissing || (missing == bestMissing && unknown < bestUnknown) {
						bestMissing, bestUnknown = missing, unknown
						winner = &matches[i]
					}
				}
