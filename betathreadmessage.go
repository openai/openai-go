...
//
// Use AnnotationUnion.AsAny() to switch on the variant.
//
...
//
// Use AnnotationDeltaUnion.AsAny() to switch on the variant.
//
// Use the methods beginning with 'As' to cast the union to one of its variants.
...
type MessageAttachment struct {
	...
	Tools []MessageAttachmentsToolsUnion `json:"tools"`
	// Metadata and presence of fields
	JSON struct {
		FileID      resp.Field
		Tools       resp.Field
...
// MessageAttachmentsToolsUnion contains all possible properties and values from
// [CodeInterpreterTool, MessageAttachmentsToolsAssistantToolsFileSearchTypeOnly].
//
// Use the methods beginning with 'As' to cast the union to one of its variants.
...
// RefusalContentBlock].
//
// Use MessageContentUnion.AsAny() to switch on the variant.
//
// Use the methods beginning with 'As' to cast the union to one of its variants.
...
// [ImageFileDeltaBlock, TextDeltaBlock, RefusalDeltaBlock, ImageURLDeltaBlock].
//
// Use MessageContentDeltaUnion.AsAny() to switch on the variant.
//
// Use the methods beginning with 'As' to cast the union to one of its variants.
...
// IsPresent returns false if the field is omitted or `null`.
func (u BetaThreadMessageNewParamsAttachmentsToolUnion) IsPresent() bool {
	return !param.IsOmitted(u) && !u.IsNull()
}
...
func (u *BetaThreadMessageNewParamsAttachmentsToolUnion) asAny() any {
...
func init() {
	apijson.RegisterUnion[BetaThreadMessageNewParamsAttachmentsToolUnion](
		"type",
		apijson.UnionVariant{
			TypeFilter:         gjson.JSON,
		...
		apijson.UnionVariant{
			TypeFilter:         gjson.JSON,
			Type:               reflect.TypeOf(BetaThreadMessageNewParamsAttachmentsToolsFileSearch{}),
...
// IsPresent returns false if the field is omitted or `null`.
func (f BetaThreadMessageNewParamsAttachmentsToolsFileSearch) IsPresent() bool {
	return !param.IsOmitted(f) && !f.IsNull()
}
